...
`pnpm run check` also runs the OpenAPI contract drift check (`scripts/check-openapi-routes.mjs`): every gateway route registration must have a matching method and path in `packages/contracts/openapi.yaml` and vice versa (a spec entry with no gateway route is also drift), and the spec itself must parse without duplicate keys. Run it directly with `pnpm run check:openapi`.
