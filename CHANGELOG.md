...
- An OpenAPI contract drift check (`pnpm run check:openapi`, also wired into `pnpm run check` through the contracts package) parses the gateway source for every route registration, parses the canonical spec with a strict YAML document (duplicate keys fail), and fails in both directions: a gateway route missing from the spec, and a spec method+path entry with no gateway route. It runs in the CI quality path via the recursive check script.
